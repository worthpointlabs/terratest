...
1. `cd test`
1. Make sure [the azure-sdk-for-go versions match](#check-go-dependencies) in [/test/go.mod](/test/go.mod) and in [test/azure/terraform_azure_acr_example_test.go](/test/azure/terraform_azure_acr_example_test.go).
1. `go build terraform_azure_acr_example_test.go`
1. `go test -v -run TestTerraformAzureACRExample`
