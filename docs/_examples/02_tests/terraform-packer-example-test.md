...
title: Terraform Packer example test
category: tests
excerpt: >-
  Use Terratest to run packer build to build an AMI with a web server installed, deploy that AMI in AWS by running terraform apply, make HTTP requests to the web server to check that it is working correctly, and run terraform destroy to undeploy the web server.
tags: ["example"]
...
Full soure code can be found here: [terraform_packer_example_test.go](https://github.com/gruntwork-io/terratest/blob/master/test/terraform_packer_example_test.go).


This test fits to the [Terraform Packer Example]({{site.baseurl}}/examples/code-examples/terraform-packer-example/).
